...
def run_cell6(features, models_data, df_noisy, df_cleaned):
    ...
    for i in range(1, len(risk_pred)):
        if risk_pred[i] >= 2 and risk_pred[i-1] < 2:
            alerts.append({
                ...
                "estimated_savings": "$150-500 (mortality prevention)",
            })
    ...
    df_feed["savings_usd"] = df_feed["savings_kg"]     * 0.45
    ...
    egg_revenue  = total_eggs * 0.12
    total_fkg    = float(df_feed["predicted_feed"].sum())
    feed_cost    = total_fkg * 0.45
    ...
    fee          = max(99, min(499, monthly_ai * 0.2))
    ...
    profitability = {
        ...
        "pricing_model": {
            ...
            "value_proposition": f"${fee:.0f}/mo generates ${monthly_ai:.0f}/mo savings",
            "tier_basic":        {"price": 99,  "features": ["Monitoring","Alerts","Daily report"]},
            "tier_pro":          {"price": 299, "features": ["Everything Basic","AI optimizer","Heat prediction","SOP"]},
            "tier_enterprise":   {"price": 499, "features": ["Everything Pro","Multi-house","API","Custom models"]},
        },
    }
...
def run_cell7(sop_ctx, gemini_key=None):
    ...
    p = 1
    if risk >= 2:
        prio.append({"priority":p,"action":f"ACTIVATE emergency cooling. THI={thi:.1f}","reason":f"Prevents {int(flock*0.003)} bird deaths/day","deadline":"IMMEDIATELY","estimated_value_usd":round(flock*0.003*5,2)}); p+=1
    if nh3 > 20:
        prio.append({"priority":p,"action":f"Increase ventilation. NH3={nh3:.1f}ppm exceeds limit","reason":"Prevents respiratory disease","deadline":"Within 30 min","estimated_value_usd":round(ee*0.02*0.12,2)}); p+=1
    ...
    if gemini_key:
        KEYS = [gemini_key,
                "AIzaSyCaFp6zGiXElcOWzK_KHG1Bo9ic6nK9I-w",
        ...
        for k in KEYS:
            try:
                ...
                prompt = (f"Poultry farm AI assistant. 2-sentence executive summary: "
                          f"THI={thi:.1f}, Risk={risk_lbl}, Temp={temp}C, "
                          f"Feed={af}g/bird, Expected={ee} eggs.")
                exec_s = model.generate_content(prompt).text.strip()
                gby = "gemini"; break
            except Exception:
                continue
...
# Full pipeline
# ─────────────────────────────────────────────────────────────────────
def run_pipeline(df_raw, gemini_key=None):
    print(f"[Pipeline] {len(df_raw)} rows received")
    df_raw     = normalize_df(df_raw)
    df_cleaned = clean_sensor_data(df_raw)
    print("[Pipeline] Cell 5 — training models…")
    features, models, _ = run_cell5(df_cleaned)
    print("[Pipeline] Cell 6 — generating predictions + JSON…")
    cell6 = run_cell6(features, models, df_raw, df_cleaned)
    print("[Pipeline] Cell 7 — SOP…")
...
def upload():
    ...
    try:
        ...
        dashboard = run_pipeline(df, gemini_key=gkey)
        ...
        print(f"[Upload] Saved {len(dashboard)} JSON files")
